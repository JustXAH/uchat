...
void mx_save_user_pic_and_get_id(t_server *serv, t_json *json,
    ...
    unique_user_pic_name = mx_db_get_filename(serv->db, json->USER_PIC_ID->valueint);
    ...
    cJSON_AddItemToObject(json->SEND, "USER_PIC_ID", json->VOICE_ID);
    send_str = cJSON_Print(json->SEND);

    write(serv->user_socket[user_index], send_str, strlen(send_str));

    // функция приёма и сохранения аудиофайла
    mx_voice_file_receiver(serv, unique_voice_name, user_index);
