...
static void add_contact_login(t_user *user, cJSON *CONTACTS_LOGIN_ARR) {
    user->contacts_login = (char **)malloc(sizeof(char *) * user->contact_count);
    for (int i = 0; i != user->contact_count; i++) {
        user->contacts_login[i] = mx_strdup(cJSON_GetArrayItem(CONTACTS_LOGIN_ARR, i)->valuestring);
    }
    user->contacts_login[user->contacts_id] = NULL;
}

static void add_contacts_id(t_user *user, cJSON *CONTACTS_ID_ARR) {
    user->contacts_id = (int *)malloc(sizeof(int) * user->contact_count);
    for (int i = 0; i != user->contact_count; i++) {
        user->contacts_id[i] = cJSON_GetArrayItem(CONTACTS_ID_ARR, i)->valueint;
    }
    user->contacts_id[user->contact_count] = 0;
}
...
void mx_add_new_contact(t_system *sys, t_user *user, t_json *json) {
    ...
    if (json->RESULT->valueint == false) {
       // вызов всплывающего текста c ошибкой
    }
    else {
        user->contact_count = json->CONTACTS_COUNT->valueint;
        ...
        add_contact_login(user, CONTACTS_LOGIN_ARR);
    }
}
