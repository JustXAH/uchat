...
void mx_gtk_window(t_system *sys, t_user *user) {
   ...
   if (!gtk_builder_add_from_file(sys->builder, "glade/msg_test1.ui", &error)) {
    ...
    gtk_widget_show(sys->reg_window);
    reg_win_init(sys);
    chat_win_init(sys);
//    gtk_main();
//    sys->login = strdup(gtk_entry_get_text(GTK_ENTRY(gtk_builder_get_object(builder, "login_field"))));
}
void gtk_chat_window(t_system *sys, t_user *user) {
    gtk_widget_hide(sys->reg_window);
    gtk_widget_show(sys->chat_window);
    chat_win_init(sys);
}
